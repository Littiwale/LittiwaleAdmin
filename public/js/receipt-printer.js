...
(function() {
  ...
  function _buildEscPosReceipt(orderData) {
    ...
    const custPhone = orderData.customerPhone || orderData.whatsappPhone || 'N/A';
    const isTakeaway= orderData.orderType === 'takeaway';
    const custAddr  = isTakeaway ? 'TAKEAWAY (SELF PICKUP)' : (orderData.customerAddress || orderData.deliveryAddress || orderData.address || 'ADDRESS NOT PROVIDED - CALL CUSTOMER BEFORE DISPATCH');
  ...
  window.openA4InvoiceModal = function(orderData) {
    ...
    const custName = orderData.customerName || 'Valued Customer';
    const custPhone = orderData.customerPhone || orderData.whatsappPhone || 'N/A';
    const custAddress = orderData.customerAddress || orderData.deliveryAddress || orderData.address || 'ADDRESS NOT PROVIDED - CALL CUSTOMER BEFORE DISPATCH';
